"""
HOG-based Face Recognizer using SVM classifier.
Extracts HOG features from detected faces and classifies them.
"""
import numpy as np
import os
import joblib
...
MODEL_PATH = os.path.join(MODEL_DIR, 'hog_svm_model.pkl')
LABELS_PATH = os.path.join(MODEL_DIR, 'label_encoder.pkl')


class HOGRecognizer:
    """
    Recognizes faces using HOG feature vectors + SVM classifier.
    """
    ...
    def _load_model(self):
        ...
        if os.path.exists(MODEL_PATH) and os.path.exists(LABELS_PATH):
            try:
                ...
                print("[HOGRecognizer] Model loaded successfully.")
            except Exception as e:
                print(f"[HOGRecognizer] Failed to load model: {e}")
                self.is_trained = False

    def train(self, features_list, labels):
        """
        Train the SVM classifier on HOG feature vectors.
        
        Args:
            features_list: List of HOG feature arrays
            labels: List of student IDs (strings)
        """
        ...
        print(f"[HOGRecognizer] Model trained on {len(labels)} samples, {len(set(labels))} students.")
        return True

    def predict(self, hog_features):
        """
        Predict student identity from HOG features.
        Filters out the synthetic negative class used in single-student training.
        ...
        try:
            features = np.array(hog_features).reshape(1, -1)
            ...
            try:
                ...
                if embeddings:
                    ...
                    for emb_record in embeddings:
                        ...
                        dot_product = np.dot(hog_features, stored_emb)
                        norm_a = np.linalg.norm(hog_features)
                    ...
                    # Use standard cosine similarity threshold (70%) for verification
                    if confidence < 0.70:
                        return None, confidence
                    return predicted_label, confidence
            except Exception as db_err:
                print(f"[HOGRecognizer] Database verification failed, falling back to SVM decision values: {db_err}")

            # ── Fallback: SVM decision function softmax/sigmoid ──
            ...
            print(f"[HOGRecognizer] Prediction error: {e}")
            return None, 0.0
